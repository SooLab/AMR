...
from amr.matcher import build_matcher
from amr.transformer import build_transformer
from amr.position_encoding import build_position_encoding
from amr.misc import accuracy
from amr.interaction.test_CQA import VSLFuser
import torchvision
...
class AMR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
                 ...
                 max_v_l=75, span_loss_type="l1", use_txt_pos=False, n_input_proj=2, aud_dim=0, clip_len = 2, stage="pretrain"):
        ...
        if stage == "distill":
            self.new_query_embed = nn.Embedding(num_queries, 2)
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None):
        ...
        if self.stage == "distill":
            hs, reference, memory, saliency_scores = self.transformer(src, ~mask, self.new_query_embed.weight, pos,self.saliency_proj1, video_length=video_length, sent_feat=src_sent, new_query_embed=self.new_query_embed.weight)
        else:
...
def build_model(args):
    ...
    if args.stage == "distill":
        pretrain_transformer = build_transformer(args)
        pretrain_position_embedding, pretrain_txt_position_embedding = build_position_encoding(args)
    ...
    if args.a_feat_dir is None:
        ...
        if args.stage == "distill":
            pretrain_model = AMR(
                ...
                pretrain_position_embedding,
                pretrain_txt_position_embedding,
                txt_dim=args.t_feat_dim,
                vid_dim=args.v_feat_dim,
                num_queries=args.num_queries,
                input_dropout=args.input_dropout,
    ...
    criterion.to(device)
    if args.stage == "distill":
        return model, criterion, pretrain_model
    else:
