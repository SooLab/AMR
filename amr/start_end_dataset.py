...
from amr.span_utils import span_xx_to_cxw
    ...
    def __getitem__(self, index):
        ...
        if self.load_labels:
            if self.dset_name == 'tvsum': 
                ...
                if "wrong_spans" in meta and self.train:
                    if len(meta["wrong_spans"]) == 0:
                        model_inputs["wrong_spans"] = self.get_span_labels([[0, 0]], ctx_l)
                    else:
                        model_inputs["wrong_spans"] = self.get_span_labels(meta["wrong_spans"], ctx_l)
                    neg_mask = torch.zeros(ctx_l)
                    for st, ed in meta["wrong_spans"]:
                        neg_mask[st//self.clip_len:ed//self.clip_len] = 1
                    model_inputs["neg_mask"] = neg_mask
...
def start_end_collate(batch):
    ...
    for k in model_inputs_keys:
        ...
        if k == "saliency_all_labels":
            ...
            continue
        if k == 'wrong_spans':
            batched_data[k] = [dict(spans=e["model_inputs"]["wrong_spans"]) for e in batch]
            continue
...
def prepare_batch_inputs(batched_model_inputs, device, non_blocking=False):
    ...
    if "neg_mask" in batched_model_inputs:
        targets['src_neg_mask']=batched_model_inputs["neg_mask"][0].to(device, non_blocking=non_blocking)

    if "wrong_spans" in batched_model_inputs:
        targets["wrong_spans"] = [
            dict(spans=e["spans"].to(device, non_blocking=non_blocking))
            for e in batched_model_inputs["wrong_spans"]
        ]
