...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=10, num_encoder_layers=6,
        ...
        decoder_layer = TransformerDecoderLayer(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before, keep_query_pos=keep_query_pos, stage=stage)
    ...
    def forward(self, src, mask, query_embed, pos_embed, saliency_proj1, video_length=None, sent_feat=None, new_query_embed=None):
        """
        Args:
            src: (batch_size, L, d)
            mask: (batch_size, L)
        ...
        if sent_feat is not None:
            tgt = tgt + sent_feat.unsqueeze(0).repeat(refpoint_embed.shape[0], 1, 1)
        ...
        if new_query_embed is not None:
            new_refpoint_embed = new_query_embed.unsqueeze(1).repeat(1, bs, 1)
            new_hs, new_references = self.decoder(tgt, memory_local, memory_key_padding_mask=mask_local,
                            pos=pos_embed_local, refpoints_unsigmoid=new_refpoint_embed, is_new_query=True)
            hs = torch.cat([hs, new_hs], dim=-2)
            references = torch.cat([references, new_references], dim=-2)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False,
        ...
        self.iou_head = None
        self.stage = decoder_layer.stage
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 ...
                 rm_self_attn_decoder=False, stage="pretrain"):
        super().__init__()
        ...
        if self.stage == "distill":
            self.linear1_active = nn.Linear(d_model, dim_feedforward)
            self.dropout_active = nn.Dropout(dropout)
            self.linear2_active = nn.Linear(dim_feedforward, d_model)
    ...
    def forward(self, tgt, memory,
        ...
        tgt = self.norm2(tgt)
        if is_new_query:
            tgt2 = self.linear2_active(self.dropout_active(self.activation(self.linear1_active(tgt))))
        else:
