...
from amr.config import TestOptions
from amr.model import build_model
from amr.span_utils import span_cxw_to_xx
from amr.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs
from amr.start_end_dataset_audio import \
    StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from amr.postprocessing_amr import PostProcessorDETR
...
def set_active_weights(model):
    param_dict = dict(model.named_parameters())
    
    for name, param in model.named_parameters():
        if '_active' in name:
            target_name = name.replace('_active', '')
...
def setup_model(opt):
    ...
    logger.info("setup model/optimizer/scheduler")
    if opt.stage == "distill":
        model, criterion, pretrain_model = build_model(opt)
    ...
    if opt.resume is not None:
        ...
        model.load_state_dict(checkpoint["model"], strict=False)
        if opt.stage == "distill":
            pretrain_model.load_state_dict(checkpoint["model"], strict=False)
        ...
        logger.info(f"Loaded model saved at epoch {checkpoint['epoch']} from checkpoint: {opt.resume}")
        if opt.stage == "distill":
            model = set_active_weights(model)
    else:
        logger.warning("If you intend to evaluate the model, please specify --resume with ckpt path")

    if opt.stage == "distill":
        return model, criterion, optimizer, lr_scheduler, pretrain_model
    else:
        return model, criterion, optimizer, lr_scheduler
